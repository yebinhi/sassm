...
def train(model, dataloader, criterion, optimizer, scheduler, num_epochs, device, model_save_path, result_folder):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in dataloader:
            ...
            optimizer.zero_grad()
            outputs = model(input1_l, input_m, input_s, input_cur)
    ...
    model_save_path = f"{model_save_path}green_transformer_model_{timestamp}.pth"
    ...
    plot_save_path = os.path.join(result_folder, f'training_loss_{timestamp}.png')
